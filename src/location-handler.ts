...
export class LocationHandler {
    ...
    public getClosestBusStop = async () => {
        try {
            ...
            if (!busStopDoc.exists) {
                ...
                // Shwo on map button
                const bus_loc = get(busStopDoc, 'd.coordinates');
    ...
    private checkBusTraffic = async (userLocation: number[]) => {
        try {
            ...
            switch (true) {
                case speed <= 10:
                    return { trafficStatus: `ติดขัดมาก`, timeInMin: `${fnb.time + 5}` };
                case speed <= 20:
                    return { trafficStatus: `ติดขัดเล็กน้อย`, timeInMin: `${fnb.time + 3}` };
                default:
                    return { trafficStatus: 'ปกติ', timeInMin: `${fnb.time}` }
            }
